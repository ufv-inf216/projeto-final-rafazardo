...
DrawPolygonComponent::DrawPolygonComponent(class GameObject* owner, std::vector<Vector2> &vertices, int drawOrder)
        :DrawComponent(owner)
        ,mVertices(vertices)
...
void DrawPolygonComponent::Draw(SDL_Renderer *renderer) {
    ...
    Vector2 cameraPos = mOwner->GetGame()->GetCamera()->GetPosition();
